...
> **更新记录：**  
> - **v2.0** 数据结构替换为标准JSON Schema；废弃5.4飞书字段映射，新增5.4 JSON Schema接口规范；28步收集重新设计；1.1.1改为通用标品描述；1.1.2新增多品牌多市场定位；去重/节点/校验/Redis全链路适配标准接口（保留原章节结构完整性）
> - v1.1 补充完整获客链路、Agent间身份传递、意图分流架构  
> - v1.2 补充28步收集设计、校验规则、完整字段映射  
> - v1.3 去掉附录图表内容  
...
|:-------|:-----|
| **唯一标识** | 用户联系方式（手机号） |
| **历史信息查询** | 通过唯一标识查 Redis/数据库，获取是否被触达过、是否有售前对话 |
...
| 用户角色 | 痛点 | 影响 |
|:---------|:-----|:-----|
| **借贷用户** | 进件流程复杂，不知道填什么 | 放弃申请，流失 |
| **借贷用户** | 中途退出后不知道如何继续 | 需要重新开始，体验差 |
...
| 编号 | 角色 | 行为 | 收益 | 优先级 |
|:-----|:-----|:-----|:-----|:------:|
| US-01 | 借贷用户 | 发消息"我要借款"触发进件流程 | 快速进入申请流程 | P0 |
| US-02 | 借贷用户 | 一步步填写姓名、手机、身份证等信息 | 不会迷失，知道填什么 | P0 |
| US-03 | 借贷用户 | 中途退出后重新发消息 | 继续上次进度，不用重新填 | P0 |
| US-04 | 借贷用户 | 输入格式错误（如手机号格式不对） | 收到错误提示，知道怎么改 | P0 |
| US-05 | 借贷用户 | 填完所有信息后确认提交 | 进件成功，等待审批结果 | P0 |
| US-06 | 借贷用户 | 审批中再次申请 | 收到提示"申请正在审核" | P0 |
...
| **去重与提交** |
| 11 | 进件层 | HTTP | 查进件记录 | 手机号去重查询 |
| 12 | 进件层 | Switch | 去重判断 | 审批中拒绝 / 已完成返回冷却期提示 |
| 13 | 进件层 | HTTP | 提交进件数据 | 新增进件记录 |
...
| **进件数据 API 权限** | 用户 | ⏳ 待确认 | 是否有 API Token |
| **进件数据后续演进方案** | 待定 | ⏳ 待确认 | 正式数据库 / 业务系统 API / Copaw 内部存储 |
| **审批通知方式** | 用户 | ⏳ 待确认 | 后续阶段，钉钉/邮件 |
